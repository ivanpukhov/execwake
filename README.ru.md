...
ExecWake 0.1.0-rc.5 — альфа-версия для Linux. Сборщик записывает:
...
В release CI проверка eBPF обязательна на Ubuntu 22.04 amd64. Задания eBPF для
Ubuntu 22.04 arm64 и Ubuntu 24.04 пока advisory: flood-тест на arm64 и
подключение tracepoint на hosted runner Ubuntu 24.04 ещё недостаточно надёжны.
Обязательные тесты `auto` на Ubuntu 24.04 проверяют запись этих ошибок запуска и
fallback на ptrace.
...
Чтобы сохранить завершённую копию по заданному пути, укажите `--output` до
разделителя argv:
...
Целевой файл не должен существовать. Исходная сессия остаётся в приватном
каталоге состояния ExecWake.
...
Компактный машиночитаемый JSON выводится в stdout так:

```sh
execwake diff --json before.sqlite3 after.sqlite3
```

Верхний уровень JSON содержит `formatVersion`, детерминированные счётчики
результата `counts` и полный объект `diff`. Текущая версия машинного формата —
1; потребителю следует отклонять неподдерживаемую версию. Если имя одного из
файлов начинается с дефиса, перед путями нужно поставить `--`.

Параметр `--exit-code` включается явно и сочетается с `--json`. Код `0` означает
отсутствие изменений, `10` — изменение сопоставимого поведения, `11` — наличие
хотя бы одной несопоставимой категории. Ошибки разбора или входных данных
возвращают обычные коды ошибки команды.
...
Этот релиз читает данные поведения из схем 9, 10 и 11 — то есть файлы от rc.1
до rc.5. Импортированные файлы остаются открытыми только для чтения. Неизвестные
более новые схемы отклоняются.
...
Об уязвимостях сообщайте по инструкции в [SECURITY.ru.md](SECURITY.ru.md). Для
обычных ошибок перед созданием issue используйте
...
артефакты для x86_64 и arm64. Package CI работает на Ubuntu 22.04 и 24.04 для
обеих архитектур. Совместимые с релизом сборки также запускаются в Debian 12, а
workflow отклоняет бинарный файл, требующий glibc новее версии 2.35.
